...
Make a formal table:  
We are scratching the percect perfect thing and I just want to make a table comparing updating and constant fmsy percent bad outcomes. I'll create the dataframe from other wrangling I did above and then make a table.  

```{r slow-bad-table, echo=FALSE}
#Dataframe with the fmsy constant prop bad values:
slow_prop <- data.frame(error_0.1 = slow1c_badavg, error_0.3 = slow3c_badavg, error_0.5 = slow5c_badavg)

#Add the row for updating fmsy values to the dataframe:
slow_prop <- add_row(slow_prop, error_0.1 = slow1_badavg, error_0.3 = slow3_badavg, error_0.5 = slow5_badavg)

#Add row names:
row.names(slow_prop) = c("constant fmsy", "updating fmsy")

slow_table <- kable(slow_prop, col.names = c("10% Error", "30% Error", "50% Error")) %>% 
...
```{r int-bad-table, echo=FALSE}
#Dataframe with the fmsy constant prop bad values:
int_prop <- data.frame(error_0.1 = int1c_badavg, error_0.3 = int3c_badavg, error_0.5 = int5c_badavg)

#Add the row for updating fmsy values to the dataframe:
int_prop <- add_row(int_prop, error_0.1 = int1_badavg, error_0.3 = int3_badavg, error_0.5 = int5_badavg)

#Add row names:
row.names(int_prop) = c("constant fmsy", "updating fmsy")

int_table <- kable(int_prop, col.names = c("10% Error", "30% Error", "50% Error")) %>% 
  kable_styling(bootstrap_options = c("striped", "bordered"))
int_table
```
...
#Add the row to the dataframe:
yield_fast2 <- add_row(yield_fast2, perfect = percent_perf, perfect_fishe = percent_perf_fishe_f, error_0.1 = percent_perf_f0.1,
...
```{r fast-bad-table, echo=FALSE}
#Dataframe with the fmsy constant prop bad values:
fast_prop <- data.frame(error_0.1 = fast1c_badavg, error_0.3 = fast3c_badavg, error_0.5 = fast5c_badavg)

#Add the row for updating fmsy values to the dataframe:
fast_prop <- add_row(fast_prop, error_0.1 = fast1_badavg, error_0.3 = fast3_badavg, error_0.5 = fast5_badavg)

#Add row names:
row.names(fast_prop) = c("constant fmsy", "updating fmsy")

fast_table <- kable(fast_prop, col.names = c("10% Error", "30% Error", "50% Error")) %>% 
  kable_styling(bootstrap_options = c("striped", "bordered"))
fast_table
```
