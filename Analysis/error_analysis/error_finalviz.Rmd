...
##Create the graph data frame - use the constant df (graph_df) from above:
graph_u <- data.frame(error = prop_100_u$error, type = rep("closed", 3), prop = prop_100_u$prop_bad, productivity = rep("updating", 3))
graph_both <- rbind(graph_df, graph_u)
```
...
##Try the other graph with the fill by type:
prop_graph_both <- ggplot(graph_both, aes(x=error, y=prop)) + 
