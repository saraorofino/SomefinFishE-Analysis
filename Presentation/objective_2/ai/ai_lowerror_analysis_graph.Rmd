...
ai_lowerror_all <- rbind(rep0_1, rep20_1, rep15_1, rep10_1, rep5_1) %>%
  mutate(assess_int = factor(int, levels = intervals)) %>%
  group_by(assess_int) %>% 
  count(status) %>% 
  spread(key = "status", value = "n") %>% 
...
##All results:
ai_graph_all <- ggplot(ai_lowerror_all, aes(x = assess_int, y = prop_bad)) +
  geom_col()
...
graph_c <- data.frame(assessment_interval = ai_lowerror$int, productivity = rep("constant", 5), prop_bad = ai_lowerror_all$prop_bad)
graph_u <- data.frame(assessment_interval = ai_lowerror_u$int, productivity = rep("updating", 5), prop_bad = ai_lowerror_all_u$prop_bad)
graph_both_all <- rbind(graph_c, graph_u) %>% 
  mutate(assess_int = rep(intervals_f, 2)) #change intervals to a factor


##Create the 1500 graph data frame - start with constant and updating separately then merge:
graph_c1500 <- data.frame(assessment_interval = ai_lowerror$int, productivity = rep("constant", 5), prop_bad = ai_lowerror$prop_bad)
graph_u1500 <- data.frame(assessment_interval = ai_lowerror$int, productivity = rep("updating", 5), prop_bad = ai_lowerror_u$prop_bad)
...
ai_1500_both <- ggplot(graph_both1500, aes(x = assess_int, y = prop_bad, fill = productivity)) +
  ...
  theme(axis.text.x=element_text(size=15), axis.text.y=element_text(size=15), plot.title = element_text(hjust = 0.5, face = "bold", size = 20), axis.title.x = element_text(face = "bold", size = 15), axis.title.y = element_text(face = "bold", size = 15), legend.title.align=0.5, panel.background = element_rect(fill = "transparent",colour = NA), plot.background = element_rect(fill = "transparent",colour = NA), legend.title=element_text(size=15), 
    ...
    legend.box.background = element_blank()) +
  geom_text(aes(label = round(prop_bad, digits = 2)), position = position_dodge(0.8), vjust = 2, size = 3.5)

ai_1500_both
ggsave(ai_1500_both, filename = "assessment2.png",  bg = "transparent")

ai_all_both <- ggplot(graph_both_all, aes(x = assess_int, y = prop_bad, fill = productivity)) +
