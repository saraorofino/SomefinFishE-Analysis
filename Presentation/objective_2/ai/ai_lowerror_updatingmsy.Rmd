...
The test will cover a range of climate impacts, initial growth rates, and initial biomasses, with a 30% reduction hcr.
...
#Baseline model with error around the f_ratio (f/fmsy) and no repetition of the assessment process:
...
sim_ai <- function(b, r, r_s, error, p, k, years, hcr){ 
  ...
  # For this run, there will be no repeating decision - make the HCR decision based on the previous years data and apply the f to the current assessment year, this f remains constant until next assessment
  for (t in 2:years) {
    ...
    results$f_ratio_p[t] <- results$f[t-1]/results$f_msy[t-1] #the perceived f ratio is based on the original fmsy from the first assessment
...
##Save CSV
Resave csv with new name after using a 30% hcr (1/21)
```{r save-csv}
...
for(i in 1:1224){
  ...
  dat$id <- i
  dat$r_0 <- rep(input_ai[[i]]$r, 100)
  dat$err <- rep(input_ai[[i]]$error, 100)
  dat$r_s <- rep(input_ai[[i]]$r_s, 100)
  dat$hcr <- rep(input_ai[[i]]$hcr, 100)
  dat$b_0 <- rep(input_ai[[i]]$b, 100)
  results_id[[i]] <- dat
}
...
#Write as CSV:
 library(here)
  write.csv(results_ai, file=file.path(here(),"Presentation/objective_2/ai/ai_5rep_lowerror_updatingfmsy.csv"),
            row.names = FALSE)
