...
graph_u_ai <- data.frame(assessment_interval = ai_props_u$assess_int, response = rep("ideal", 5),
                      prop_good = ai_props_u$prop_good)
graph_pre_ai <- data.frame(assessment_interval = ai_props_p$assess_int, response = rep("precautionary", 5),
                        prop_good = ai_props_p$prop_good)
graph_both_ai <- rbind(graph_c_ai, graph_u_ai, graph_pre_ai)
```
...
ai_all <- ggplot(graph_both_ai, aes(x = assessment_interval, y = prop_good, fill = response)) +
  geom_col(position = "dodge", alpha = 0.8) + 
