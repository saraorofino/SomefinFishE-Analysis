...
err_u_all <- read_csv(file=file.path(here(), "/Analysis/fmsy_analysis/fmsy_u_all.csv"))
```
...
```{r wrangle-new-data}
err_u_all[is.na(err_u_all)] <- "misc" #turn status NA into misc

# Calculate proportions for graphing:
err_props_u_all <- err_u_all %>% 
  group_by(error) %>%
  count(status) %>% 
  spread(key = "status", value = "n") %>% 
  mutate(simulations = sum(closed + over + good + misc)) %>% 
...
#Want to have error as a factor so that the graphs have a discrete x-axis:
err_props_u_all$error <- as.factor(err_props_u_all$error)
```
