...
hcr_rep5u <- read_csv(file=file.path(here(),"/Results/hcrs/hcrs_updatingfmsy_5rep_lowerror.csv"))
hcr_rep10u <- read_csv(file=file.path(here(),"/Results/hcrs/hcrs_updatingfmsy_10rep_lowerror.csv"))
```
...
hcr_props_u <- rbind(rep5u, rep10u) %>% 
  group_by(hcr) %>%
  count(status) %>% 
  spread(key = "status", value = "n") %>% 
...
hcr_both <- ggplot(graph_both, aes(x = hcr, y = prop_good, fill = productivity)) +
  ...
  scale_fill_manual(values = c("#BC6435", "#079EDF")) +
  theme_light()+
  coord_cartesian( ylim=c(0,1), expand = FALSE ) +
  scale_x_reverse(labels=c("0.5" = "50%","0.6" = "40%" ,"0.7" = "30%","0.8" = "20%","0.9" = "10%","1.0" = "0%")) +
  labs(title = "Fishing Reduction Response", x = "Reduction Amount (%)", y = "Proportion Healthy")+
...
Note: Remove the legend by adding this code at the very end of the theme() command:  legend.position = "none" 
I'm using the Ai graph for the legend so this one doesn't need one  
