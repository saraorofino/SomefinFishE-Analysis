...
#Baseline model with error around the f_ratio (f/fmsy) and repetition of the assessment process every 20 years for the duration of the simulation:
...
sim_ai <- function(b, r, r_s, error, p, k, years, hcr){ 
  ...
  # Repeat on 20 year intervals - make the HCR decision based on the previous years data and apply the f to the current assessment year, this f remains constant until next assessment
  for (t in 2:years) {
...
##Save CSV
Save new csv file with 30% hcr (1/21)
```{r save-csv}
...
#Write as CSV:
# library(here)
#  write.csv(results_ai, file=file.path(here(),"Results/assessment_intervals/ai_updatingfmsy_20rep_hcr30.csv"),
#            row.names = FALSE)
