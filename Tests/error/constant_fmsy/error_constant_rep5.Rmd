...
check_result_5c <- sim_5c(b=1500, r=0.2, r_s=-0.01767, error=0.10, p=0.2, k=10000, years=100)
```
...
col_names <- c("b", "c", "year", "r", "f", "f_msy", "f_ratio", "f_ratio_p", "f_ratio_err", "id", "r_0", "error", "r_s",
               "ratio_p_cat", "ratio_err_cat", "decision_made", "correct_decision")
...
for(i in 1:1224){
  ...
  dat$id <- i
  dat$r_0 <- rep(input_5c[[i]]$r, 100)
  dat$err <- rep(input_5c[[i]]$error, 100)
  dat$r_s <- rep(input_5c[[i]]$r_s, 100)
  results_id[[i]] <- dat
}

#Bind all the experiments together
results_rep5c <- bind_rows(results_id)

#Mutate columns to compare f_ratio to f_ratio_err 
# results_rep5c_edit <- results_rep5c %>%
# mutate(ratio_cat = case_when(results_rep5c$V7 == 0 ~ 0,
#                                results_rep5c$V7 <= 1 & results_rep5c$V7 != 0 ~ 1,
#                                results_rep5c$V7 > 1 & results_rep5c$V7 < 2 ~ 2,
#                                results_rep5c$V7 > 2 ~ 3)) %>% 
#   mutate(ratio_err_cat = case_when(results_rep5c$V9 == 0 ~ 0,
#                                results_rep5c$V9 > 0 & results_rep5c$V9 <= 1 ~ 1,
#                                results_rep5c$V9 > 1 & results_rep5c$V9 < 2 ~ 2,
#                                results_rep5c$V9 > 2 ~ 3)) %>%
...
#Mutate columns to compare f_ratio_p and f_ratio_err
results_rep5c_edit <- results_rep5c %>%
mutate(ratio_p_cat = case_when(results_rep5c$V8 == 0 ~ 0,
                               results_rep5c$V8 <= 1 & results_rep5c$V8 != 0 ~ 1,
                               results_rep5c$V8 > 1 & results_rep5c$V8 < 2 ~ 2,
                               results_rep5c$V8 > 2 ~ 3)) %>% 
  mutate(ratio_err_cat = case_when(results_rep5c$V9 == 0 ~ 0,
                               results_rep5c$V9 > 0 & results_rep5c$V9 <= 1 ~ 1,
                               results_rep5c$V9 > 1 & results_rep5c$V9 < 2 ~ 2,
                               results_rep5c$V9 > 2 ~ 3)) %>%
  mutate(decision_made = case_when(ratio_p_cat == ratio_err_cat & ratio_p_cat !=0 ~ "yes",
                                      ratio_p_cat != ratio_err_cat ~ "yes",
                                      ratio_p_cat == ratio_err_cat & ratio_p_cat == 0 ~ "no")) %>% 
  mutate(correct_decision = case_when(ratio_p_cat == ratio_err_cat & ratio_p_cat !=0 ~ "yes",
                                      ratio_p_cat != ratio_err_cat ~ "no",
                                      ratio_p_cat == ratio_err_cat & ratio_p_cat == 0 ~ "fail"))
#Update the column names:
colnames(results_rep5c_edit) <- col_names

#Write as CSV:
# library(here)
# write.csv(results_rep5c_edit,
#           file=file.path(here(),"/Results/error/error_constant_rep5_hcr30_p.csv"),
#           row.names = FALSE)
