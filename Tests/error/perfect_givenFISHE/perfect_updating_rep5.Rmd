...
In this experiment we are simulating perfect management under the FISHE process. In the case of looking at error this means that the managers would always make the correct decision on the harvest control rule.

The true Fmsy is adjusting over time as climate change alters productivity and during assessment years the managers are updating their estimates of Fmsy to reflect productivity changes. 
...
Assumptions:
  - Perfect management under FISHE would be always making the correct decision (the decision f_ratio would have you make) despite error in your assessment methods
...
sim_5u_perf <- function(b, r, r_s, p, k, years){
  
  results <- data.frame(
    b = rep(NA, years), c = rep(NA, years), 
    year = 1:years, r = rep(NA, years), f = rep(NA, years),
    f_msy = rep(NA, years), f_ratio = rep(NA, years)) #Setup the results dataframe 
  ...
  for (t in 2:years) {
    ...
    #Decisions for f this year based on the ratio in the previous year 
...
check_result_5u_perf <- sim_5u_perf(b=6000, r=0.2, r_s=-0.01767, p=0.2, k=10000, years=100)
```
...
for(i in 1:408){
  results[[i]] <- sim_5u_perf(b=input_5u_perf[[i]]$b, r=input_5u_perf[[i]]$r,
                             r_s=input_5u_perf[[i]]$r_s, p=input_5u_perf[[i]]$p,
                             k=input_5u_perf[[i]]$k,years=input_5u_perf[[i]]$years)
}

```

##Save CSV
Re-saved CSV file on 1/13 after updating the HCR rule to a 30% reduction (SO)
```{r save-csv}

#Create the list of column names:

col_names <- c("b", "c", "year", "r", "f", "f_msy", "f_ratio", "id", "r_0", "r_s")
...
for(i in 1:408){
  dat <- as.data.frame(matrix(unlist(results[i]), ncol = 7))
  dat$id <- i
  dat$r_0 <- rep(input_5u_perf[[i]]$r, 100)
  dat$r_s <- rep(input_5u_perf[[i]]$r_s, 100)
  results_id[[i]] <- dat
}

#Bind all the experiments together
results_rep5u_perf <- bind_rows(results_id)

#Update the column names:
colnames(results_rep5u_perf) <- col_names

#Write as CSV:
# library(here)
# write.csv(results_rep5u_perf,
#           file=file.path(here(),"/Results/error/perfect_updating_rep5_hcr30.csv"),
#           row.names = FALSE)
