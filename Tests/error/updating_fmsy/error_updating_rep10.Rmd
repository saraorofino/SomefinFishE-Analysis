...
##Fmsy is updating with climate change and managers are updating their perceived Fmsy during assessment years
...
check_result_10u <- sim_10u(b=1500, r=0.2, r_s=-0.01767, error=0.10, p=0.2, k=10000, years=100)
```
...
results_rep10u_edit <- results_rep10u %>% 
  mutate(ratio_cat = case_when(results_rep10u$V7 == 0 ~ 0,
                               results_rep10u$V7 <= 1 & results_rep10u$V7 != 0 ~ 1,
                               results_rep10u$V7 > 1 & results_rep10u$V7 < 2 ~ 2,
                               results_rep10u$V7 > 2 ~ 3)) %>% 
  mutate(ratio_err_cat = case_when(results_rep10u$V8 == 0 ~ 0,
                               results_rep10u$V8 > 0 & results_rep10u$V8 <= 1 ~ 1,
                               results_rep10u$V8 > 1 & results_rep10u$V8 < 2 ~ 2,
                               results_rep10u$V8 > 2 ~ 3)) %>%
...
#Write as CSV:
 ...
 #           file=file.path(here(),"/Results/error/error_updating_rep10_hcr30.csv"),
 #           row.names = FALSE)
...
#Write as CSV - make sure to change the CSV file name!!
 # library(here)
 # write.csv(results_rep10u_edit,
 #           file=file.path(here(),"/Results/error/error_updating_rep10_.1-.25.csv"),
 #           row.names = FALSE)
