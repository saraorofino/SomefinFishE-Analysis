...
sim_msy5c <- function(b, r, r_s, error, p, k, years){ 
...
check_result_msy5c <- sim_msy5c(b=6000, r=0.2, r_s=-0.01767, error=0.3, p=0.2, k=10000, years=100)
```
...
for(i in 1:408){
  results[[i]] <- sim_msy5c(b=input_msy5c[[i]]$b, r=input_msy5c[[i]]$r,
                             r_s=input_msy5c[[i]]$r_s, p=input_msy5c[[i]]$p,
                             k=input_msy5c[[i]]$k, years=input_msy5c[[i]]$years, 
                             error = input_msy5c[[i]]$error)
}
...
for(i in 1:408){
  ...
  dat$id <- i
  dat$r_0 <- rep(input_msy5c[[i]]$r, 100)
  dat$r_s <- rep(input_msy5c[[i]]$r_s, 100)
  results_id[[i]] <- dat
}

#Bind all the experiments together
results_msy5c <- bind_rows(results_id)

#Mutate columns to compare f_ratio to f_ratio_err 
results_msy5c_edit <- results_msy5c %>%
mutate(ratio_cat = case_when(results_msy5c$V7 == 0 ~ 0,
                               results_msy5c$V7 <= 1 & results_msy5c$V7 != 0 ~ 1,
                               results_msy5c$V7 > 1 & results_msy5c$V7 < 2 ~ 2,
                               results_msy5c$V7 > 2 ~ 3)) %>% 
  mutate(ratio_err_cat = case_when(results_msy5c$V9 == 0 ~ 0,
                               results_msy5c$V9 > 0 & results_msy5c$V9 <= 1 ~ 1,
                               results_msy5c$V9 > 1 & results_msy5c$V9 < 2 ~ 2,
                               results_msy5c$V9 > 2 ~ 3)) %>%
...
colnames(results_msy5c_edit) <- col_names

#Write as CSV:
# library(here)
#  write.csv(results_msy5c_edit,
#            file=file.path(here(),"/Results/fmsy_update/msy_constant_rep5.csv"),
#            row.names = FALSE)
