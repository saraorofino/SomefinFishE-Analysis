...
sim_u <- function(b, r, r_s, error, p, k, years, hcr){ #hcr term is added into the model to run a cross list of a number of different hcr possibilities
  ...
  if(results$f_ratio_err[1] >= 2){
       results$f[1] = hcr*f_int #Not closing the fishery in year 1
   } 
...
for(i in 1:4080){
  results[[i]] <- sim_u(b=input_msy_u[[i]]$b, r=input_msy_u[[i]]$r,
                             r_s=input_msy_u[[i]]$r_s, p=input_msy_u[[i]]$p,
                             k=input_msy_u[[i]]$k, years=input_msy_u[[i]]$years, 
                             error = input_msy_u[[i]]$error, hcr = input_msy_u[[i]]$hcr)
}
...
for(i in 1:4080){
  ...
  dat$id <- i
  dat$r_0 <- rep(input_msy_u[[i]]$r, 100)
  dat$err <- rep(input_msy_u[[i]]$error, 100)
  dat$r_s <- rep(input_msy_u[[i]]$r_s, 100)
  dat$hcr <- rep(input_msy_u[[i]]$hcr, 100)
  dat$b_0 <- rep(input_msy_u[[i]]$b, 100)
  results_id[[i]] <- dat
}

#Bind all the experiments together
results_u <- bind_rows(results_id)

#Update the column names:
colnames(results_u) <- col_names

#Write as CSV:
library(here)
 write.csv(results_u,
           file=file.path(here(),"/Results/fmsy_update/higherror/higherr_u_20.csv"),
           row.names = FALSE)
