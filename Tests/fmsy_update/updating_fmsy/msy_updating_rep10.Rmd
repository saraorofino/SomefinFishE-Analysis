...
In this experiment, the true Fmsy is adjusting over time as climate change alters productivity and managers are updating their perceived Fmsy in assessment years 
...
##Fmsy is updating with climate change and managers are updating perceived Fmsy in assessment years

sim_msy10u <- function(b, r, r_s, error, p, k, years){
...
check_result_msy10u <- sim_msy10u(b=6000, r=0.2, r_s=-0.01767, error=0.3, p=0.2, k=10000, years=100)
```
...
for(i in 1:408){
  results[[i]] <- sim_msy10u(b=input_msy10u[[i]]$b, r=input_msy10u[[i]]$r,
                             r_s=input_msy10u[[i]]$r_s, p=input_msy10u[[i]]$p,
                             k=input_msy10u[[i]]$k, years=input_msy10u[[i]]$years, 
                             error = input_msy10u[[i]]$error)
}
...
col_names <- c("b", "c", "year", "r", "f", "f_msy", "f_ratio", "f_ratio_err", "id", "r_0", "r_s",
...
for(i in 1:408){
  ...
  dat$id <- i
  dat$r_0 <- rep(input_msy10u[[i]]$r, 100)
  dat$r_s <- rep(input_msy10u[[i]]$r_s, 100)
  results_id[[i]] <- dat
}

#Bind all the experiments together
results_msy10u <- bind_rows(results_id)

#Mutate columns to compare f_ratio to f_ratio_err 
results_msy10u_edit <- results_msy10u %>%
  mutate(ratio_cat = case_when(results_msy10u$V7 == 0 ~ 0,
                               results_msy10u$V7 <= 1 & results_msy10u$V7 != 0 ~ 1,
                               results_msy10u$V7 > 1 & results_msy10u$V7 < 2 ~ 2,
                               results_msy10u$V7 > 2 ~ 3)) %>% 
  mutate(ratio_err_cat = case_when(results_msy10u$V8 == 0 ~ 0,
                               results_msy10u$V8 > 0 & results_msy10u$V8 <= 1 ~ 1,
                               results_msy10u$V8 > 1 & results_msy10u$V8 < 2 ~ 2,
                               results_msy10u$V8 > 2 ~ 3)) %>%
...
colnames(results_msy10u_edit) <- col_names

#Write as CSV:
# library(here)
#  write.csv(results_msy10u_edit,
#            file=file.path(here(),"/Results/fmsy_update/msy_updating_rep10.csv"),
#            row.names = FALSE)
