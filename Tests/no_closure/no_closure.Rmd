...
sim_noclose <- function(b, r, r_s, error, p, k, years, hcr, over_hcr){
  ...
  if(results$f_ratio_err[1] >= 2){
       results$f[1] = over_hcr*f_int #Reduce by the over_cutback value
   } 
  ...
  for (t in 2:years) {
    ...
    if(results$f_ratio_err[t] >= 2){
       results$f[t] = over_hcr*f_int #Reduce by the over_cutback value (same as keeping over_cutback%) 
   } 
...
for(i in 1:4080){
  results[[i]] <- sim_noclose(b=input_noclose[[i]]$b, r=input_noclose[[i]]$r,
                             ...
                             over_hcr = input_noclose[[i]]$over_hcr)
}
...
for(i in 1:4080){
  ...
  dat$over_hcr <- rep(input_noclose[[i]]$over_hcr, 100)
...
library(here)
write.csv(results_noclose, file=file.path(here(),"/Results/no_closure/miderror/miderr_1_95.csv"),
           row.names = FALSE)
