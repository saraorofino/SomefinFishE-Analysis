...
Including a new variable called over_cutback which will represent the cutback amount if the fishery is over the limit  
...
sim_noclose <- function(b, r, r_s, error, p, k, years, hcr, over_cutback){
  ...
  if(results$f_ratio_err[1] >= 2){
       results$f[1] = over_cutback*f_int #Reduce by the over_cutback value
   } 
  ...
  for (t in 2:years) {
    ...
    if(results$f_ratio_err[t] >= 2){
       results$f[t] = over_cutback*f_int #Reduce by the over_cutback value (same as keeping over_cutback%) 
   } 
...
list_noclose <- list(
  ...
  over_cutback = seq(0.2,0.05, -0.05) #80% cutback to 95% cutback
)
...
for(i in 1:16320){
  results[[i]] <- sim_noclose(b=input_noclose[[i]]$b, r=input_noclose[[i]]$r,
                             ...
                             over_cutback = input_noclose[[i]]$over_cutback)
}
...
col_names <- c("b", "c", "year", "r", "f", "f_msy", "f_ratio", "f_ratio_p", "f_ratio_err", "id", "r_0", "error", "r_s", "hcr", "over_cutback", "b_0")
...
for(i in 1:16320){
  ...
  dat$over_cutback <- rep(input_noclose[[i]]$over_cutback, 100)
...
#Write as CSV:
library(here)
write.csv(results_noclose, file=file.path(here(),"/Results/no_closure/miderror/miderr_20_run1.csv"),
           row.names = FALSE)
