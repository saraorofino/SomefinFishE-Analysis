...
##Setup
Want to look at what happens when there is no closure of the fishery, instead we will look at the effects of implementing a 95% cutback.  

This time the model with use updating productivity  

The files are too large to run all the error and assessment intervals together, so we'll run them one at a time  
...
sim_noclose <- function(b, r, r_s, error, p, k, years, hcr){
  ...
  results$f_ratio_p[1] = results$f_ratio[1] #this is the perceived f ratio by the fisheries managers - will update as productivity changes
  ...
  for (t in 2:years) {
    ...
    results$f_ratio_p[t] <- results$f[t-1]/results$f_msy[t-1] #updating perceived based on updated fmsy
...
library(here)
write.csv(results_noclose,
          file=file.path(here(),"/Results/no_closure/updating_fmsy/higherror/higherr_20_95.csv"),
           row.names = FALSE)
