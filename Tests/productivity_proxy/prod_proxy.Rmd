...
sim_msy_proxy <- function(b, r, r_s, r_p_s, error, p, k, years, hcr){ 
...
check_result_proxy <- sim_msy_proxy(b=6000, r=0.2, r_s=-0.00967, r_p_s = -0.00367, error=0.3, p=0.2, k=10000, years=100, hcr = 0.9)
```
...
for(i in 1:2448){
  results[[i]] <- sim_msy_proxy(b=input_msy_proxy[[i]]$b, r=input_msy_proxy[[i]]$r,
...
library(here)
write.csv(results_noclose, file=file.path(here(),"/Results/no_closure/miderror/miderr_1_80.csv"),
           row.names = FALSE)
