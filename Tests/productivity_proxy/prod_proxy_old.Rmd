---
title: "Productivity Proxy Old Model"
...
sim_proxy_old <- function(b, r, r_s, r_p_s, error, p, k, years, hcr){ 
  ...
  results$f_ratio_p[1] = f_int/results$f_msy_p[1] #this is the perceived f ratio by the fisheries managers - with climate change, this perceived ratio will not have an adjusted fmsy as productivity changes
  ...
  if(results$f_ratio_err[1] >= 2){
    ...
    if(results$f_ratio_err[1] > 1.1 & results$f_ratio_err[1] < 2){
       results$f[1] = hcr*f_int #Reduce by 10%
   }  
  ...
  for (t in 2:years) {
    ...
    results$f_ratio_p[t] <- results$f[t-1]/results$f_msy_p[t-1] #the perceived f ratio is based on the original fmsy calculation
    ...
    if(results$f_ratio_err[t] > 1.1 & results$f_ratio_err[t] < 2){
       results$f[t] = hcr*results$f[t-1] #Reduce by 10%
   }  
...
check_result_proxy_old <- sim_proxy_old(b=6000, r=0.2, r_s=-0.00967, r_p_s = -0.00367, error=0.3, p=0.2, k=10000, years=100, hcr = 0.9)
```
...
list_msy_proxy <- list(
  b = c(1500, 4000, 6000),
  r = seq(0.1, .8, .1),
  r_s = seq(-0.01767, 0, 0.002),
  r_p_s = c(0, -0.00167, -0.00367, -0.00567),
  error = 0.5,
  p = 0.2,
  k = 10000,
  years = 100,
  hcr = seq(0.9, 0.50, -0.1)
)
...
for(i in 1:4320){
  results[[i]] <- sim_proxy_old(b=input_msy_proxy[[i]]$b, r=input_msy_proxy[[i]]$r,
...
col_names <- c("b", "c", "year", "r", "r_p", "f", "f_msy", "f_msy_p", "f_ratio", "f_ratio_p", "f_ratio_err", "id", "r_0", "error", "r_s", "r_p_s", "hcr", "b_0")
...
library(here)
write.csv(results_proxy, file=file.path(here(),"/Results/productivity_proxy/old_model/higherror/proxy_old_higherr1.csv"),
           row.names = FALSE)
